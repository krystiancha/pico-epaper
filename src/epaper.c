...
extern const uint8_t lut_faster_vcom[], lut_faster_w2w[], lut_faster_b2w[], lut_faster_w2b[], lut_faster_b2b[];
extern const uint8_t lut_fastest_vcom[], lut_fastest_w2w[], lut_fastest_b2w[], lut_fastest_w2b[], lut_fastest_b2b[];
...
void epaper_update(struct epaper *display, bool partial) {
    ...
    epaper_write_array(display, 1, !partial ? lut_faster_vcom : lut_fastest_vcom, 44);
    epaper_write(display, 0, 1, EPAPER_LUTWW);
    epaper_write_array(display, 1, !partial ? lut_faster_w2w : lut_fastest_w2w, 42);
    epaper_write(display, 0, 1, EPAPER_LUTBW);
    epaper_write_array(display, 1, !partial ? lut_faster_b2w : lut_fastest_b2w, 42);
    epaper_write(display, 0, 1, EPAPER_LUTWB);
    epaper_write_array(display, 1, !partial ? lut_faster_w2b : lut_fastest_w2b, 42);
    epaper_write(display, 0, 1, EPAPER_LUTBB);
    epaper_write_array(display, 1, !partial ? lut_faster_b2b : lut_fastest_b2b, 42);
